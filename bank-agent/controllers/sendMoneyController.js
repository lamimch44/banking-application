...
async function sendMoneyV(req, res, next){
     const findUser = await Agent.findOne({email: req.body.email})
     const findSender = await Agent.findOne({email: res.locals.loggedInUser.email})
     if(findUser){
        if(findUser.email !== findSender.email){
         ...
         if(req.body.amount > 19){
              ...
              const updateSender = await Agent.updateOne({email: findSender.email}, {balance: minusSender})
             const updateReceiver =  await Agent.updateOne({email: findUser.email}, {balance: plusReceiver})
             ...
             const transactionOb = Transaction({
                ...
                sendername: findSender.storename,
                receivername: findUser.storename,
